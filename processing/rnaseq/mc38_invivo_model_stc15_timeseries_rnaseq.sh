### in vivo RNA-seq sample processing

wd="mc38_invivo"

sample_info="processing/rnaseq/mc38_invivo_model_stc15_timeseries/sample_info/mc38invivo_sample_info.csv"
...
IntronAnnotationGTF="Mmus_ensembl_canonical_protein_coding_introns.saf"
RunonAnnotationGTF="Mmus_ensembl_canonical_protein_coding_runon.gtf"

bamfiles=$(ls ${wd}/alignment/STAR_bam/*.bam)

featurecounts_introns="${ir_featurecounts_directory}/featurecounts_introns_mc38_invivo"
featurecounts_runon="${ro_featurecounts_directory}/featurecounts_runon_mc38_invivo"

##Map to introns SAF
featureCounts -p --countReadPairs -s 2 -T 16 -F "SAF" -f -O -M -a ${IntronAnnotationGTF} -o ${featurecounts_introns} ${bamfiles}

##Map to runon GTF
featureCounts -p --countReadPairs -s 2 -T 16 -O -M -a ${RunonAnnotationGTF} -o ${featurecounts_runon} ${bamfiles}
