...
mane_select_exon_gtf = gtf[gtf$tag %in% "MANE_Select" & gtf$type %in% "exon" & seqnames(gtf) %in% levels(seqnames(gtf))[1:24]]
s1 = split(mane_select_exon_gtf, mane_select_exon_gtf$gene_id)
...
write.table(introns, file="Hsap_mane_select_introns.saf", quote = F, sep="\t", row.names = F)

################################################################################
#### Annotate intron annotations with type and width of upstream exon

introns = as_tibble(read.table(file="Hsap_mane_select_introns.saf", sep="\t", header = T)) 
introns1 = introns %>%
...
exons_gr = gtf[gtf$tag %in% "MANE_Select" & gtf$type %in% "exon"]
...
hsap_mane_select_introns_with_updn_exons = intron_with_updn_exon

save(hsap_mane_select_introns_with_updn_exons, file = "hsap_mane_select_introns_with_updn_exons.Rda")
